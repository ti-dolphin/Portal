...
export default function GroupForm({callback, grupo, usuarios, isEdit, setOpen}){
    ...
    const AtualizaSessao = async () => {
        ...
        var grupos = await api.get('usuario-grupo?target[]=usuario_id&target_value[]='+usuarioAux.id+'&target[]=empresa_id&target_value[]='+usuarioAux.empresa_id);

        grupos.data.map((grupo) => {
            usuarioAux.grupos.push(grupo.grupo_id)
        })
