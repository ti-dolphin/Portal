...
router.get("/", function (req, res) {
    ...
    Material.select(fields, target).then((rows, fields) => {
        res.status(200).json(rows);
    }).catch(err => {
        console.log("ERRO: linha 34 matorial_routes " + err);
        res.status(500).send({
            erro: err
        });
    });
...
router.post("/", function (req, res) {
    Material.insert(req.body).then((rows, fields) => {
        res.status(201).json({
        ...
        console.log("ERRO: linha 52 matorial_routes " + err);
        res.status(500).send({
            message: "Erro!"
        });
    });
...
router.put("/", function (req, res) {
    Material.update(req.body).then((rows, fields) => {
        res.json(rows);
        res.status(201);
    }).catch((err) => {
        console.log("ERRO: linha 67 matorial_routes " + err);
        res.status(500).send({
            erro: String(err)
        });
    });
...
router.get('/:id', function (req, res) {
    Material.select(null, [{
        ...
        console.log("ERRO: linha 84 matorial_routes " + err);
        res.status(500).send({
            erro: err
        });
    });
